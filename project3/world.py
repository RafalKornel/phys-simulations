...
class World:
    def __init__(self, planets, static_objects = [], size = 4, G = 0.01, temp = 2.5, k = 1, dt = 0.0001):
        self.G = G
    ...
    def initialize_world(self, wall_len):
    ...
    def select_closest(self, planet):
        ...
        for other in self.planets:
            d = self.d_torus(other.pos, planet.pos)
            if d < self.radius and planet is not other:
                selected.append((other, d))
    ...
    def draw2(self, i):
        ...
        pl.title(f"t:{self.temp:.4f} | p:{self.press:.1f}")
        i = str(i).rjust(6, '0')
